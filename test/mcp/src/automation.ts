...
export async function getServer(appService: ApplicationService): Promise<Server> {
	const server = new McpServer({
		name: 'HiTechAi Studio Code Automation Server',
		version: '1.0.0',
		title: 'An MCP Server that can interact with a local build of HiTechAi Studio Code. Used for verifying UI behavior.'
	}, { capabilities: { logging: {} } });

	server.tool(
		'vscode_automation_start',
		'Start HiTechAi Studio Code Build. If workspacePath is not provided, HiTechAi Studio Code will open with the last used workspace or an empty window.',
		{
		...
		async ({ recordVideo, workspacePath }) => {
			...
			return {
				content: [{
					type: 'text' as const,
					text: app ? `HiTechAi Studio Code started successfully${workspacePath ? ` with workspace: ${workspacePath}` : ''}` : `Failed to start HiTechAi Studio Code`
				}]
			};
		}
	);

	// Apply all HiTechAi Studio Code automation tools using the modular structure
