...
export function applyCoreTools(server: McpServer, appService: ApplicationService): RegisteredTool[] {
	...
	tools.push(server.tool(
		...
		'Restart HiTechAi Studio Code with optional workspace or folder and extra command-line arguments',
		{
		...
		async ({ workspaceOrFolder, extraArgs }) => {
			...
			return {
				content: [{
					type: 'text' as const,
					text: `HiTechAi Studio Code restarted successfully${workspaceText}${argsText}`
				}]
			};
		}
	));
	...
	tools.push(server.tool(
		...
		'Stop the HiTechAi Studio Code application',
		async () => {
			const app = await appService.getOrCreateApplication();
			...
			return {
				content: [{
					type: 'text' as const,
					text: 'HiTechAi Studio Code stopped successfully'
				}]
			};
		}
	));
	...
	// 	'Get the quality/build type of HiTechAi Studio Code (Dev, Insiders, Stable, etc.)',
	// 	async () => {
	// 		const info = {
	// 			quality: app.quality,
	// 			remote: app.remote,
	...
	// 			content: [{
	// 				type: 'text' as const,
	// 				text: `HiTechAi Studio Code Info:\n${JSON.stringify(info, null, 2)}`
	// 			}]
	// 		};
	// 	}
	// );
	...
	// 	'Send a keybinding to HiTechAi Studio Code (e.g., ctrl+shift+p, cmd+s)',
	// 	{
