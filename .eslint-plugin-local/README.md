...
For example, here's a configuration that enables the no `test.only` rule in all `*.test.ts` files in the HiTechAi Studio Code repo:
...
The fixes can be run using `npx eslint --fix` in the HiTechAi Studio Code repo
