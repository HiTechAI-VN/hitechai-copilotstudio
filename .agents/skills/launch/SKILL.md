---
name: launch
description: "Launch and automate HiTechAi Studio Code (Code OSS) using @playwright/cli via Chrome DevTools Protocol. Use when you need to interact with the HiTechAi Studio Code UI, automate the chat panel, test UI features, or take screenshots of HiTechAi Studio Code. Triggers include 'automate HiTechAi Studio Code', 'interact with chat', 'test the UI', 'take a screenshot', 'launch Code OSS with debugging'."
metadata:
  allowed-tools: Bash(npx @playwright/cli:*)
---

# HiTechAi Studio Code Automation

Automate HiTechAi Studio Code (Code OSS) using `@playwright/cli`. HiTechAi Studio Code is built on Electron/Chromium and exposes a Chrome DevTools Protocol (CDP) port that `@playwright/cli` can attach to, enabling the same snapshot-interact workflow used for web pages.
...
- **For Code OSS (HiTechAi Studio Code dev build):** The repo must be built before launching. `./scripts/code.sh` runs the build automatically if needed, or set `VSCODE_SKIP_PRELAUNCH=1` to skip the compile step if you've already built.
- **CSS selectors are internal implementation details.** Selectors like `.interactive-input-part`, `.interactive-input-editor`, and `.part.auxiliarybar` used in `eval` commands are HiTechAi Studio Code internals that may change across versions. If they stop working, use `npx @playwright/cli snapshot` to re-discover the current DOM structure.
...
## Launching Code OSS (HiTechAi Studio Code Dev Build)

The HiTechAi Studio Code repository includes `scripts/code.sh` which launches Code OSS from source. It passes all arguments through to the Electron binary, so `--remote-debugging-port` works directly:

```bash
cd <repo-root>  # the root of your HiTechAi Studio Code checkout
./scripts/code.sh --remote-debugging-port=9224
```
...
- Code OSS uses the default user data directory. Unlike HiTechAi Studio Code Insiders, you don't typically need `--user-data-dir` since there's usually only one Code OSS instance running.
...
**Tips:**
- The `--agents` flag launches the Agents workbench instead of the standard HiTechAi Studio Code workbench.
- Set `VSCODE_SKIP_PRELAUNCH=1` to skip the compile step if you've already built.

## Launching HiTechAi Studio Code Extensions for Debugging

To debug a HiTechAi Studio Code extension via npx @playwright/cli, launch HiTechAi Studio Code Insiders with `--extensionDevelopmentPath` and `--remote-debugging-port`. Use `--user-data-dir` to avoid conflicting with an already-running instance.
...
# Launch HiTechAi Studio Code Insiders with the extension and CDP
code-insiders \
  --extensionDevelopmentPath="<extension-repo-root>" \
  --remote-debugging-port=9223 \
  --user-data-dir=/tmp/vscode-ext-debug

# Wait for HiTechAi Studio Code to start, retry until attached
...
- `--user-data-dir=<path>` — uses a separate profile so it starts a new process instead of sending to an existing HiTechAi Studio Code instance

**Without `--user-data-dir`**, HiTechAi Studio Code detects the running instance, forwards the args to it, and exits immediately — you'll see "Sent env to running instance. Terminating..." and CDP never starts.
...
HiTechAi Studio Code uses Monaco Editor for all text inputs including the Copilot Chat input. Monaco editors require specific npx @playwright/cli techniques — standard `click`, `fill`, and `type` commands may not work depending on the HiTechAi Studio Code build.

### The Universal Pattern: Focus via Keyboard Shortcut + `press`

This works on **all** HiTechAi Studio Code builds (Code OSS, Insiders, stable):
...
This shortcut focuses the chat input and sets `document.activeElement` to a `DIV` with class `native-edit-context` — HiTechAi Studio Code's native text editing surface that correctly processes key events from `npx @playwright/cli press`.

### `fill <ref>` — Works on Some Builds

On HiTechAi Studio Code Insiders (extension debug mode), `fill` handles focus and input in one step:
...
| Method | HiTechAi Studio Code Insiders | Code OSS |
|--------|-----------------|----------|
...
### Screenshot Tips for HiTechAi Studio Code
...
- HiTechAi Studio Code uses multiple webviews. Use `npx @playwright/cli tab-list` to list targets and switch to the right one with `npx @playwright/cli tab-select <index>`
...
- `fill` and `type` work on HiTechAi Studio Code Insiders but **silently fail on Code OSS** — they complete without error but no text appears. The `press`-per-key approach works universally.
