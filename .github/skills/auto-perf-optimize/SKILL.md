...
description: "Run agent-driven HiTechAi Studio Code performance or memory investigations. Use when asked to launch Code OSS, automate a HiTechAi Studio Code scenario, run the Chat memory smoke runner, capture renderer heap snapshots, take workflow screenshots, compare run summaries, or drive a repeatable scenario before heap-snapshot analysis."
metadata:
  allowed-tools: Bash(npx @playwright/cli:*)
---

# HiTechAi Studio Code Performance Workflow

Drive a repeatable HiTechAi Studio Code scenario, collect memory/performance artifacts, verify that the scenario actually happened, then hand the resulting heap snapshots to the generic heap-snapshot-analysis skill when object-level investigation is needed.

## When to Use

- User describes a HiTechAi Studio Code workflow and asks whether it leaks or grows memory
- User asks the agent to launch HiTechAi Studio Code, drive a scenario, and capture heap snapshots
...
- User wants a new automation runner for a non-Chat HiTechAi Studio Code scenario
...
## Non-Chat HiTechAi Studio Code Scenarios
