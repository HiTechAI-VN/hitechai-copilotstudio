...
- Analyzing HiTechAi Studio Code user timing marks like `code/didResolveTextFileEditorModel` (trace files)
- Understanding multi-process behavior (Browser, Renderer, GPU processes in trace files)

## Detecting File Type

- **`.cpuprofile`**: Top-level JSON with `nodes`, `samples`, `timeDeltas` keys. Created by the HiTechAi Studio Code profiler.
...
- `$vscode`: Optional HiTechAi Studio Code metadata
...
DevTools traces are the future of perf tracing for HiTechAi Studio Code. They are created from the built-in Electron/Chrome DevTools Performance tab and contain far more information than `.cpuprofile` files.
...
| `blink.user_timing` | HiTechAi Studio Code performance marks (e.g. `code/willResolveTextFileEditorModel`, `code/didResolveTextFileEditorModel`) |
...
|---------|------|------------|
| **Renderer** (pid varies) | HiTechAi Studio Code's renderer process -- where JS runs | `CrRendererMain` (main thread) |
...
For HiTechAi Studio Code perf analysis, focus on the **Renderer process, CrRendererMain thread** -- this is where JavaScript execution, layout, and painting happen.
...
HiTechAi Studio Code emits `performance.mark()` calls that appear as `blink.user_timing` events. These are the most direct way to measure HiTechAi Studio Code-specific milestones:
...
- **User timing marks**: HiTechAi Studio Code milestone events and their timestamps
...
- Use `args.data.url` in `FunctionCall` and `EvaluateScript` events to map back to HiTechAi Studio Code source files (paths like `vscode-file://vscode-app/Users/.../out/vs/...`).
