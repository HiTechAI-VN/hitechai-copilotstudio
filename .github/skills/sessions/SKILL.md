...
vs/workbench     ← Standard HiTechAi Studio Code workbench
...
### 2.3 How It Differs from HiTechAi Studio Code

| Aspect | HiTechAi Studio Code Workbench | Agents Window |
|--------|-------------------|----------------------|
...
Views and contributions that should only appear in the agents window (not in regular HiTechAi Studio Code) use `WindowVisibility.Sessions` in their registration.
...
2. Lean on existing HiTechAi Studio Code services (`IPromptsService`, `IMcpService`, `IChatService`)
3. Browser compatibility required — no Node.js APIs
4. Active worktree comes from `IActiveSessionService`

### 10.5 Validation

1. Check `HiTechAi Studio Code - Build` task output for compilation errors before declaring work complete
