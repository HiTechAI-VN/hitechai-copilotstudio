---
name: Demonstrate
description: Agent for demonstrating HiTechAi Studio Code features
...
5. Start HiTechAi Studio Code automation using `vscode_automation_start`
...
## Pointers for Controlling HiTechAi Studio Code

- **Prefer `vscode_automation_*` tools over `browser_*` tools** when available - these are designed specifically for HiTechAi Studio Code interactions and provide more reliable control. For example:
...
**Monaco editors (used throughout HiTechAi Studio Code) DO NOT work with standard Playwright methods like `.click()` on textareas or `.fill()` / `.type()`**
