...
export interface IStorageMainService {
	 ...
	 * across HiTechAi Studio Code and Agents app.
	 */
...
export class StorageMainService extends Disposable implements IStorageMainService {
	...
	private createApplicationSharedStorage(): IStorageMain {
		...
		// In HiTechAi Studio Code: reuse the own application storage (keys are local)
		...
		if (hostUserRoamingDataHome) {
			// - In the Agents App: create a storage backed by the host (HiTechAi Studio Code)
			//   app's application DB so keys are found even if HiTechAi Studio Code hasn't
