		...
		const server = createServer((req, res) => {
			if (req.url === '/json/list' || req.url === '/json') {
				...
				res.end(JSON.stringify([{
					description: 'HiTechAi Studio Code Renderer',
					devtoolsFrontendUrl: '',
					id: ident,
					title: 'HiTechAi Studio Code Renderer',
					type: 'page',
					url: 'vscode://renderer',
				...
				res.setHeader('Content-Type', 'application/json');
				res.end(JSON.stringify({
					'Browser': 'HiTechAi Studio Code Renderer',
		...
		const { server, wsUrl, port } = await this.openCdpServer(ident, listener => {
			...
			store.add(listener.onData(rawData => {
				...
				if (debugRenderer) {
					...
					const targetInfo = { targetId: ident, type: 'page', title: 'HiTechAi Studio Code Renderer', url: 'vscode://renderer' };
