...
export class CopilotAgent extends Disposable implements IAgent {
	...
	private async _ensureClient(): Promise<ICopilotClient> {
		...
		const clientStarting = (async () => {
			...
			// Build a clean env for the CLI subprocess, stripping Electron/HiTechAi Studio Code vars
			...
			// Add HiTechAi Studio Code's built-in ripgrep to PATH so the CLI subprocess can find it.
