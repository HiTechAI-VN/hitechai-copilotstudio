...
export class SSHRemoteAgentHostMainService extends Disposable implements ISSHRemoteAgentHostMainService {
	...
	private async _ensureCLIInstalled(client: SSHClient, platform: { os: string; arch: string }, reportProgress: (message: string) => void): Promise<void> {
		...
		if (code === 0) {
			this._logService.info(`${LOG_PREFIX} HiTechAi Studio Code CLI already installed on remote`);
			return;
		}

		reportProgress(localize('sshProgressDownloadingCLI', "Installing HiTechAi Studio Code CLI on remote..."));
		...
		this._logService.info(`${LOG_PREFIX} HiTechAi Studio Code CLI installed successfully`);
	}
}
