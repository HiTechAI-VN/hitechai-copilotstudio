...
The current authentication mechanism is imperative and HiTechAi Studio Code-specific:

1. The renderer discovers agents via `listAgents()` and checks `IAgentDescriptor.requiresAuth`.
2. It obtains a GitHub OAuth token from HiTechAi Studio Code's built-in authentication service.
...
### Client-side (HiTechAi Studio Code renderer)
...
2. **Phase B**: Update HiTechAi Studio Code renderer to use `authenticate` instead of `setAuthToken`. External clients can start using the new flow.
