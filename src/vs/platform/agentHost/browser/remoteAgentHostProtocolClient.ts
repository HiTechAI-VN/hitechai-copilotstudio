...
export class RemoteAgentHostProtocolClient extends Disposable implements IAgentConnection {
	...
	private _handleMessage(msg: ProtocolMessage): void {
		if (isJsonRpcRequest(msg)) {
			...
			switch (msg.method) {
				case 'action': {
					// Protocol envelope → HiTechAi Studio Code envelope (superset of action types)
	...
	/** Send a JSON-RPC request for a HiTechAi Studio Code extension method (not in the protocol spec). */
