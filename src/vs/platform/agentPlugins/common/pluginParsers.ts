 ...
 * using HiTechAi Studio Code's `${env:VAR}` colon-delimited syntax.
 */
const BARE_ENV_VAR_RE = /\$\{(?![A-Za-z]+:)([A-Z_][A-Z0-9_]*)\}/g;

/**
 * Converts bare `${VAR}` environment-variable references to HiTechAi Studio Code `${env:VAR}` syntax.
 */
...
/**
 * Maps known hook type identifiers from all formats (HiTechAi Studio Code PascalCase,
 * Copilot CLI camelCase, Claude PascalCase) to canonical identifiers.
 */
const HOOK_TYPE_MAP: Record<string, string> = {
	// PascalCase (HiTechAi Studio Code / Claude)
