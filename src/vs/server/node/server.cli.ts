...
/*
 * Implements a standalone CLI app that opens HiTechAi Studio Code from a remote terminal.
 ...
 *  - In external terminals for WSL this calls HiTechAi Studio Code on the Windows side.
 *    The HiTechAi Studio Code desktop executable path is passed in env VSCODE_CLIENT_COMMAND.
 */
...
export async function main(desc: ProductDescription, args: string[]): Promise<void> {
	if (!cliPipe && !cliCommand) {
		console.log('Command is only available in WSL or inside a HiTechAi Studio Code terminal.');
		return;
	}
...
function fatal(message: string, err: unknown): void {
	console.error('Unable to connect to HiTechAi Studio Code server: ' + message);
	console.error(err);
	process.exit(1);
}
