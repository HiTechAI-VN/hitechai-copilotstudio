...
export class ExtensionEnablementService extends Disposable implements IWorkbenchExtensionEnablementService {
	...
	private readonly _hasSeparateDefaultChatExtensions: boolean;
	...
	constructor(
		...
		this._hasSeparateDefaultChatExtensions = !!this._completionsExtensionId && !!this._chatExtensionId && this._completionsExtensionId !== this._chatExtensionId;
		const unificationExtensions = this._hasSeparateDefaultChatExtensions && this._completionsExtensionId && this._chatExtensionId ? [this._completionsExtensionId, this._chatExtensionId] : [];
		...
		if (this._hasSeparateDefaultChatExtensions && chatExtensionId && !this.environmentService.isSessionsWindow && !this.environmentService.skipBuiltinExtensions?.some(id => id.toLowerCase() === chatExtensionId)) {
			...
			if (!builtinChatExtensionEnablementMigration) {
				...
				if (context) {
					if (context.value.state.completed) {
						...
						if (this._isDisabledGlobally({ id: chatExtensionId })) {
	...
	private _isDisabledByUnification(identifier: IExtensionIdentifier): boolean {
		if (!this._hasSeparateDefaultChatExtensions || !this._completionsExtensionId) {
			return false;
		}
