...
class McpServerItemRenderer implements IListRenderer<IMcpServerItemEntry | IMcpBuiltinItemEntry, IMcpServerItemTemplateData> {
	...
	renderElement(element: IMcpServerItemEntry | IMcpBuiltinItemEntry, index: number, templateData: IMcpServerItemTemplateData): void {
		...
		templateData.disposables.add(this.hoverService.setupManagedHover(
			...
			localize('bridgedHover', "This server is managed by HiTechAi Studio Code and forwarded to all compatible agent sessions."),
		));
	...
	private filterServers(): void {
		...
		if (extensionServers.length > 0) {
			...
			entries.push({
				...
				isFirst,
				description: localize('extensionGroupDescription', "MCP servers contributed by installed HiTechAi Studio Code extensions."),
				collapsed,
			});
			if (!collapsed) {
		...
		if (otherBuiltinServers.length > 0) {
			...
			entries.push({
				...
				isFirst,
				description: localize('builtInGroupDescription', "MCP servers built into HiTechAi Studio Code. These are available automatically."),
				collapsed,
			});
			if (!collapsed) {
