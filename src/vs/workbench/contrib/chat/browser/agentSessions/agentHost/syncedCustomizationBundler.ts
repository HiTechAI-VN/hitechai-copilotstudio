...
const DISPLAY_NAME = 'HiTechAi Studio Code Synced Data';

const MANIFEST_CONTENT = JSON.stringify({
	name: DISPLAY_NAME,
	description: 'Customization data synced from HiTechAi Studio Code',
}, null, '\t');
	...
	async bundle(files: readonly ISyncableFile[]): Promise<IBundleResult | undefined> {
		...
		return {
			ref: {
				...
				description: `${syncable.length} customization(s) synced from HiTechAi Studio Code`,
				nonce,
			},
		};
	}
