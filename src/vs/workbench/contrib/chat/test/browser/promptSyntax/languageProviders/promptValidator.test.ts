...
suite('PromptValidator', () => {
	...
	suite('agents', () => {
		...
		test('unknown attribute in agent file', async () => {
			...
			assert.deepStrictEqual(
				markers.map(m => ({ severity: m.severity, message: m.message, tags: m.tags })),
				[
					{ severity: MarkerSeverity.Hint, message: `Attribute 'applyTo' is not supported in HiTechAi Studio Code agent files. Supported: agents, argument-hint, description, disable-model-invocation, github, handoffs, hooks, model, name, target, tools, user-invocable.`, tags: [MarkerTag.Unnecessary] },
				]
			);
		});
		...
		test('vscode target agent validates normally', async () => {
			const content = [
				'---',
				'description: "HiTechAi Studio Code agent"',
				'target: vscode',
				'model: MAE 4.1',
				`tools: ['tool1', 'tool2']`,
				'---',
			...
			const markers = await validate(content, PromptsType.agent);
			assert.deepStrictEqual(markers, [], 'HiTechAi Studio Code target should validate normally');
		});
		...
		test('vscode target agent with mcp-servers and github-tools', async () => {
			...
			const messages = markers.map(m => m.message);
			assert.deepStrictEqual(messages, [
				'Attribute \'mcp-servers\' is ignored when running locally in HiTechAi Studio Code.',
	...
	suite('claude rules', () => {
		...
		test('claude rules with unknown attribute shows unnecessary hint', async () => {
			...
			assert.strictEqual(markers[0].severity, MarkerSeverity.Hint);
			assert.deepStrictEqual(markers[0].tags, [MarkerTag.Unnecessary]);
			assert.ok(markers[0].message.includes(`Attribute 'applyTo' is not supported in rules files by HiTechAi Studio Code agents.`));
		});
	...
	suite('claude agents', () => {
		...
		test('Claude agent tools are not validated against HiTechAi Studio Code tool registry', async () => {
			// Claude tool names (Edit, Grep, etc.) don't exist in HiTechAi Studio Code's tool registry
			...
			const markers = await validate(content, PromptsType.agent, claudeAgentUri);
			assert.deepStrictEqual(markers, [], 'Claude tools should not be validated against HiTechAi Studio Code registry');
		});
		...
		test('Claude agent does not validate handoffs or agents attributes', async () => {
			// handoffs and agents are HiTechAi Studio Code-specific; they shouldn't be validated for Claude
			const content = [
				'---',
				'name: test-agent',
				'description: Test',
