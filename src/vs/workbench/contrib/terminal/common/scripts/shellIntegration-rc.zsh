...
# This variable allows the shell to both detect that HiTechAi Studio Code's shell integration is enabled as well
...
if [ -z "${VSCODE_PYTHON_AUTOACTIVATE_GUARD:-}" ]; then
	...
	if [ -n "${VSCODE_PYTHON_ZSH_ACTIVATE:-}" ] && [ "$TERM_PROGRAM" = "vscode" ]; then
		...
		if ! builtin eval "$VSCODE_PYTHON_ZSH_ACTIVATE"; then
			...
			builtin printf '\x1b[0m\x1b[7m * \x1b[0;103m HiTechAi Studio Code Python zsh activation failed with exit code %d \x1b[0m' "$__vsc_activation_status"
		fi
	fi
