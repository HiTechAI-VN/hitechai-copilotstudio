...
if (-not $env:VSCODE_PYTHON_AUTOACTIVATE_GUARD) {
	...
	if ($env:VSCODE_PYTHON_PWSH_ACTIVATE -and $env:TERM_PROGRAM -eq 'vscode') {
		...
		try {
			...
			$activationError = $_
			Write-Host "`e[0m`e[7m * `e[0;103m HiTechAi Studio Code Python powershell activation failed with exit code $($activationError.Exception.Message) `e[0m"
		}
	}
...
if ($Global:__VSCodeState.IsA11yMode -eq "1") {
	...
	if (-not $hasScreenReaderParam -and $PSVersionTable.PSVersion -ge "7.0") {
		...
		# When only HiTechAi Studio Code's accessibility mode is enabled (no OS screen reader),
		...
		# Import HiTechAi Studio Code's bundled PSReadLine 2.4.3 which has EnableScreenReaderMode
...
# Set always on key handlers which map to default HiTechAi Studio Code keybindings
