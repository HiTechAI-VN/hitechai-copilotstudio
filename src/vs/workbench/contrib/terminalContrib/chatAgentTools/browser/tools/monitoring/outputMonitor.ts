...
export class OutputMonitor extends Disposable implements IOutputMonitor {
	...
	private async _handleIdleState(token: CancellationToken): Promise<{ resources?: ILinkLocation[]; shouldContinuePolling: boolean; output?: string }> {
		...
		// Check for HiTechAi Studio Code's task finish messages (like "press any key to close the terminal").
		// If the execution is a task and the output contains a HiTechAi Studio Code task finish message,
		...
		if (isTask && detectsVSCodeTaskFinishMessage(outputTail)) {
			this._logService.trace('OutputMonitor: Idle -> HiTechAi Studio Code task finish message detected, stopping');
...
/**
 * Localized task finish messages from HiTechAi Studio Code's terminalTaskSystem.
...
/**
 * Detects HiTechAi Studio Code's specific task completion messages like:
 * - "Press any key to close the terminal."
 * - "Terminal will be reused by tasks, press any key to close it."
 * These appear when a task finishes and should be ignored if the task is done.
 * Note: These messages may be prefixed with " * " by HiTechAi Studio Code and may have line wrapping
...
/**
 * Detects generic "press any key" prompts from scripts (not HiTechAi Studio Code task messages).
...
export function detectsGenericPressAnyKeyPattern(cursorLine: string): boolean {
	// Match "press any key" but exclude HiTechAi Studio Code task-specific messages
