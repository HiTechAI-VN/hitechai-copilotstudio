...
`);
	...
	async getChangelog(token: CancellationToken): Promise<string> {
		...
		if (this.type === ExtensionType.System) {
			return Promise.resolve(`Please check the [HiTechAi Studio Code Release Notes](command:${ShowCurrentReleaseNotesActionId}) for changes to the built-in extensions.`);
		}
