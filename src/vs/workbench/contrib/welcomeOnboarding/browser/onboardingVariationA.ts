...
export class OnboardingVariationA extends Disposable implements IOnboardingService {
	...
	show(): void {
		...
		this.overlay.setAttribute('aria-label', localize('onboarding.a.aria', "Welcome to HiTechAi Studio Code"));
	...
	private _renderSignInStep(container: HTMLElement): void {
		...
		title.textContent = localize('onboarding.signIn.heroTitle', "Welcome to HiTechAi Studio Code");
	...
	private _renderPersonalizeSubtitle(container: HTMLElement): void {
		...
		container.append(
			...
			localize('onboarding.personalize.tip.suffix', " to access all HiTechAi Studio Code commands."),
		);
	}
	...
	private async _applyKeymap(keymapId: string): Promise<void> {
		...
		if (!keymap?.extensionId) {
			return; // HiTechAi Studio Code default, nothing to install
		}
