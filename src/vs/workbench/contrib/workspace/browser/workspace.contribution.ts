 ...
 * a folder/workspace also persists trust to the parent HiTechAi Studio Code install.
...
function getSessionsWindowTrustNote(environmentService: IWorkbenchEnvironmentService, productService: IProductService, isWorkspace: boolean): string | undefined {
	...
	const parentAppName = productService.quality === 'stable'
		...
		: productService.quality === 'insider'
			? 'HiTechAi Studio Code Insiders'
			: productService.quality === 'exploration'
				? 'HiTechAi Studio Code Exploration'
	...
	.registerConfiguration({
		...
		properties: {
			[WORKSPACE_TRUST_ENABLED]: {
				type: 'boolean',
				default: true,
				description: localize('workspace.trust.description', "Controls whether or not Workspace Trust is enabled within HiTechAi Studio Code."),
			...
			[WORKSPACE_TRUST_EMPTY_WINDOW]: {
				type: 'boolean',
				default: true,
				markdownDescription: localize('workspace.trust.emptyWindow.description', "Controls whether or not the empty window is trusted by default within HiTechAi Studio Code. When used with `#{0}#`, you can enable the full functionality of HiTechAi Studio Code without prompting in an empty window.", WORKSPACE_TRUST_UNTRUSTED_FILES),
				tags: [WORKSPACE_TRUST_SETTING_TAG],
				scope: ConfigurationScope.APPLICATION
			}
		}
