...
/**
 * Describes a color theme from the parent HiTechAi Studio Code installation.
 */
...
class ThemeImporterService extends Disposable implements IThemeImporterService {
	...
	private async _resolveVSCodeTheme(): Promise<IParentThemeInfo | undefined> {
		try {
			...
			this.logService.warn('[VSCodeThemeImporter] Failed to resolve HiTechAi Studio Code theme:', err);
			return undefined;
		}
	}

	/**
	 * Scans the host HiTechAi Studio Code's extensions directory to find which extension
