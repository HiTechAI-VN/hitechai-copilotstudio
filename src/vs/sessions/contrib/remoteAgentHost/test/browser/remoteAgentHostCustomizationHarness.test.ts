...
suite('RemoteAgentHostCustomizationHarness', () => {
	...
	test('provider hides synthetic bundle but still expands its contents', async () => {
		...
		const bundleRef: CustomizationRef = { uri: bundleUri, displayName: 'HiTechAi Studio Code Synced Data', nonce: 'abc' };
		const synced: SessionCustomization = {
		...
		assert.ok(!items.some(i => i.name === 'HiTechAi Studio Code Synced Data'), 'synthetic bundle should be hidden');
