...
export class SessionsWalkthroughOverlay extends Disposable {
	 ...
	 * account resolution (e.g. HiTechAi Studio Code signing in) cannot auto-dismiss
	...
	private _renderThemeStep(): void {
		...
		// Start resolving the parent HiTechAi Studio Code theme during the fade-out
		...
		stepDisposables.add(disposableTimeout(async () => {
			...
			// Only show the HiTechAi Studio Code theme option if the parent theme is different from the 4 onboarding themes
	...
	private _renderThemeStepContent(stepDisposables: DisposableStore, parentThemeSettingsId: string | undefined): void {
		...
		// Show a HiTechAi Studio Code theme option as a radio-style button inside the radiogroup
		if (parentThemeSettingsId) {
			const parentName = this.productService.embedded?.nameShort ?? 'HiTechAi Studio Code';
