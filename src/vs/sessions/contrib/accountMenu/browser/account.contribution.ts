	...
	if (state.type === StateType.AvailableForDownload) {
		...
		if (!hasCrossAppCoordinator) {
			const { confirmed } = await dialogService.confirm({
				message: localize('sessionsUpdateFromVSCode.title', "Update from HiTechAi Studio Code"),
				detail: localize('sessionsUpdateFromVSCode.detail', "This will close the Agents app and open HiTechAi Studio Code so you can install the update.\n\nLaunch Agents again after the update is complete."),
				primaryButton: localize('sessionsUpdateFromVSCode.open', "Close and Open HiTechAi Studio Code"),
			});
