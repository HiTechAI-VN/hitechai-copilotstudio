...
The management editor lives in `vs/workbench` (shared between core HiTechAi Studio Code and sessions):
...
├── aiCustomizationWorkspaceService.ts          # Core HiTechAi Studio Code workspace service impl
...
| Property / Method | Core HiTechAi Studio Code | Agent Sessions Window |
|----------|-------------|----------|
...
- **Factory functions** — `createVSCodeHarnessDescriptor`, `createCliHarnessDescriptor`, `createClaudeHarnessDescriptor`. The HiTechAi Studio Code harness receives `[PromptsStorage.extension, BUILTIN_STORAGE]` as extras; CLI and Claude in core receive `[]` (no extension source). Sessions CLI receives `[BUILTIN_STORAGE]`.
...
In core HiTechAi Studio Code, all three harnesses are registered but CLI and Claude only appear when their respective agents are registered (`requiredAgentId` checked via `IChatAgentService`). HiTechAi Studio Code is the default.
...
**Core HiTechAi Studio Code filter behavior:**
...
### Built-in Extension Grouping (Core HiTechAi Studio Code)

In core HiTechAi Studio Code, customization items contributed by the default chat extension (`productService.defaultChatAgent.chatExtensionId`, typically `GitHub.copilot-chat`) are grouped under the "Built-in" header in the management editor list widget, separate from third-party "Extensions".
...
- **Creation targets**: `getSourceFolders()` override replaces HiTechAi Studio Code profile user roots with `~/.copilot/{subfolder}` for CLI compatibility
