//-----------------------------------------------------------------------------
// Copyright (c) Pho Tue SoftWare Solutions JSC.  All rights reserved.
//-----------------------------------------------------------------------------

using System.Runtime.CompilerServices;

[assembly: InternalsVisibleTo("Microsoft.PowerPlatformLS.UnitTests" + PublicKeyConstants.InternalsVisibleToSuffix)]

// We don't depend on ObjectModel, so we need to copy over PublicKeyConstants.
internal static class PublicKeyConstants
{
#if STRONG_NAMING_ENABLED
    public const string InternalsVisibleToSuffix = ", PublicKey=" + PublicKey;
#else
    // Strong naming can be disabled for local builds, enabling a CoreFramework team developer to test
    // local builds without having to manually disable strong name verification for the entire machine.
    public const string InternalsVisibleToSuffix = "";
#endif
    public const string PublicKeyToken = "31bf3856ad364e35";
    public const string PublicKey =
        "0024000004800000940000000602000000240000525341310004000001000100b5fc90e7027f67" +
        "871e773a8fde8938c81dd402ba65b9201d60593e96c492651e889cc13f1415ebb53fac1131ae0b" +
        "d333c5ee6021672d9718ea31a8aebd0da0072f25d87dba6fc90ffd598ed4da35e44c398c454307" +
        "e8e33b8426143daec9f596836f97c8f74750e5975c64e2189f45def46b2a2b1247adc3652bf5c3" +
        "08055da9";
}