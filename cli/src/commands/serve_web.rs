...
/// Number of seconds where, if there's no connections to a HiTechAi Studio Code server,
...
/// served the latest version of the HiTechAi Studio Code server whenever they load the
/// page. The HiTechAi Studio Code server prefixes all assets and connections it loads with
/// its version string, so existing clients can continue to get served even
/// while new clients get new HiTechAi Studio Code Server versions.
...
/// Gets the release info from the HiTechAi Studio Code path prefix, which is in the
...
impl ConnectionManager {
	...
	/// Gets the StartData for the a version of the HiTechAi Studio Code server, triggering
