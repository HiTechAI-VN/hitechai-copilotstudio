		...
		switch (args[i]) {
			...
			case '--help': case '-h':
				console.log([
					...
					'  --build <path|ver>  Path to HiTechAi Studio Code build, or a version to download',
					...
					'  --test-setting <k=v> Set a HiTechAi Studio Code setting override for test build only',
					'  --baseline-setting <k=v> Set a HiTechAi Studio Code setting override for baseline build only',
...
/**
 * Build a local production (bundled) HiTechAi Studio Code package using `gulp vscode`.
 ...
 * @param {Record<string, any>} [settingsOverrides] - custom HiTechAi Studio Code settings
...
async function runOnce(electronPath, scenario, mockServer, verbose, runIndex, runDir, role, settingsOverrides, runOpts) {
	...
	// Stable:       .vscode-test/vscode-darwin-arm64-1.115.0/HiTechAi Studio Code.app/.../Electron → "1.115.0"
	...
	// Read the trace file written by HiTechAi Studio Code on exit via --trace-startup-file
	/** @type {Array<any>} */
	let traceEvents = [];
	try {
		...
		// Trace file may not exist if HiTechAi Studio Code crashed before shutdown
	}
