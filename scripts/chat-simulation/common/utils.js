...
/**
 * Derive the HiTechAi Studio Code repo root from an Electron executable path.
...
/**
 * Returns true if the string looks like a HiTechAi Studio Code version or commit hash
...
/**
 * Get the built-in extensions directory for a HiTechAi Studio Code executable.
...
async function resolveBuild(buildArg) {
	...
	if (isVersionString(buildArg)) {
		console.log(`[chat-simulation] Downloading HiTechAi Studio Code ${buildArg}...`);
...
/**
 * Pre-seed the HiTechAi Studio Code storage database to prevent the
...
/**
 * Build the environment variables for launching HiTechAi Studio Code with the mock server.
...
/**
 * Build the default HiTechAi Studio Code launch args.
 * @param {string} userDataDir
 * @param {string} extDir
...
/**
 * Write HiTechAi Studio Code settings that point the copilot extension at the mock server.
 * @param {string} userDataDir
 * @param {{ url: string }} mockServer
...
// -- HiTechAi Studio Code launch via CDP --------------------------------------------------
...
/**
 * Wait until HiTechAi Studio Code exposes its CDP endpoint.
 * @param {number} port
...
/**
 * Launch HiTechAi Studio Code via child_process and connect via CDP.
 * Works with dev builds, insiders, and stable releases.
 *
 * @param {string} executable - Path to the HiTechAi Studio Code executable (Electron binary or CLI)
...
async function launchVSCode(executable, launchArgs, env, opts = {}) {
	...
	child.once('exit', (code, signal) => {
		if (!exitError) {
			exitError = new Error(`HiTechAi Studio Code exited before CDP connected (code=${code} signal=${signal})`);
		}
	});
