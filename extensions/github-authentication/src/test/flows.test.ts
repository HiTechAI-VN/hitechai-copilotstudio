...
suite('getFlows', () => {
	...
	const testCases: Array<{ label: string; query: IFlowQuery; expectedFlows: Flows[] }> = [
		{
			label: 'HiTechAi Studio Code Desktop. Local filesystem. GitHub.com',
			query: {
		...
		{
			label: 'HiTechAi Studio Code Desktop. Local filesystem. GitHub Hosted Enterprise',
			query: {
				extensionHost: ExtensionHost.Local,
				isSupportedClient: true,
		...
		{
			label: 'HiTechAi Studio Code Desktop. Local filesystem. GitHub Enterprise Server',
			query: {
				extensionHost: ExtensionHost.Local,
				isSupportedClient: true,
	...
	suite('preferDeviceCodeFlow configuration', () => {
		...
		test('returns device code flow first when preferDeviceCodeFlow is true - HiTechAi Studio Code Desktop', async () => {
			const config = vscode.workspace.getConfiguration('github-authentication');
